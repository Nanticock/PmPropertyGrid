...
namespace detail
{
    namespace
    {
        ...
        const CompleteObjectLocator *read_col(const void *obj) noexcept
        {
            ...
            // A vbtable is an array of int32_t.  Entry [0] is the byte offset from the
            // vbptr back to the start of the subobject that contains it (0 or negative).
            // We require entry [0] == 0 (vbptr at subobject start), which lets us use
            // obj directly as the base for computing virtual-base addresses below.
            // Entries [1..N] are byte offsets from the vbptr to each virtual base.
